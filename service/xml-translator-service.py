...
class DataAccess:
    ...
    def get_xml(self, path):
        return self.__get_all_xmls(path)
...
def get(path):
    global root_key
    root_key = request.args["root_key"]
    global element_key
    element_key = request.args["element_key"]
    global use_id_key_from_source
    use_id_key_from_source = str_to_bool(request.args["use_id_key_from_source"]) == True
    if use_id_key_from_source:
        global id_key_from_source
        id_key_from_source = request.args["id_key"]

    entities = data_access_layer.get_xml(path)
    return Response(
